...
function launchChapter(id){
	...
	window.location.assign(window.location.hostname+launchData.page);
}

function navPage(num){
	//if (document.cookie == simple_interest_data.cookieid){
		window.location.assign(window.location.hostname+simple_interest_data.pages[num-2]);
	//}
}
...
function drop(ev) {
  ...
  if(data == ev.target.getAttribute("for")){
